...
class Application {
    ...
    function checkWakeReason() {
        ...
        switch (hardware.wakereason()) {
            ...
            default :
                ...
                _boot = true;
                server.log("Wake in " + BOOT_TIMER_SEC + " seconds")
                imp.wakeup(BOOT_TIMER_SEC, function() {
                    _boot = false;
                    server.log("Boot Powering down after " + BOOT_TIMER_SEC);
                    powerDown();
                }.bindenv(this));
        }
    ...
    function checkConnectionTime(value = null) {
        ...
        if (!_connecting) {
            ...
            if (connected || timeToConnect()) {
                ...
                server.log("Connected or Time to Connect");
                ...
                // next reading time
                server.log("Not connected -  Power Down");
                powerDown();
            }
        } else {
    ...
    function readingsAckHandler(msg) {
        ...
        server.log("Reading Ack - Power Down");
        powerDown();
    }
    ...
    function powerDown() {
        ...
        server.log("Power down _boot=" + _boot + " timer=" + timer);
        ...
        if (!_boot && timer > 2) {
            ...
            server.log("Schedule next ready " + timer);
            setWakeup(timer);
        }
    }
    ...
    function failHandler() {
        ...
        server.log("Fail handle - Power Down");
        powerDown();
    }
